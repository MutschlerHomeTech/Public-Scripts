...
# DATE     : 8-15-2014
# EDIT     : 8-15-2014
# COMMENT  : GUI interface to delete user profiles from remote desktop session host server       
#
...
ForEach ($x in $objListBox.SelectedItems) {
    ...
    ForEach ($computer in $Computers) {
        Try {
            (Get-WmiObject -ComputerName $computer Win32_UserProfile | Where-Object {$_.LocalPath -eq $selecteduser}).Delete()
